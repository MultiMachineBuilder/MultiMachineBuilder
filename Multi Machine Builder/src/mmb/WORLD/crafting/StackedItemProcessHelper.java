 ...
 */
public class StackedItemProcessHelper{
	@Nonnull private final StackedProcessingRecipeGroup recipes;
	...
	public StackedProcessingRecipe underway;
	 ...
	 */
	public StackedItemProcessHelper(StackedProcessingRecipeGroup recipes, Inventory input, Inventory output,
	...
	public CycleResult cycle() {
		if(underway == null || progress > underway.energy) {
			...
			for(ItemRecord ir: input) {
				...
				StackedProcessingRecipe candidate = recipes.recipes.get(ir.item());
				if(candidate == null) {
					//Recipe does not exist
					continue loop;
				}
				if(ir.amount() < candidate.amount) {
				...
				int extracted = ir.extract(candidate.amount);
				if(extracted >= 1) {
					//Extracted
					progress = 0;
					underway = candidate;
	 ...
	 * @param helper
	 */
	public void set(StackedItemProcessHelper helper) {
