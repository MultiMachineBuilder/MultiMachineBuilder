...
public class ElectroLuckyItemProcessHelper{
	...
	public ElectroLuckySimpleProcessingRecipe underway;
	...
	public CycleResult cycle() {
		if(underway == null || progress > underway.energy) {
			...
			for(ItemRecord ir: input) {
				...
				hasAttempted = true;
				ElectroLuckySimpleProcessingRecipe candidate = recipes.recipes.get(ir.item());
				if(candidate == null) {
			...
			if(progress > currRequired) {
				...
				//Eject the luck
				underway.luck().produceResults(output.createWriter());
				underway = null;
	 ...
	 * @param helper
	 */
	public void set(ElectroLuckyItemProcessHelper helper) {
