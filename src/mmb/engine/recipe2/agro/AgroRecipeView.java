 ...
 * @author oskar
 */
public class AgroRecipeView extends RecipeView<@NN AgroRecipe>{
	...
	@Override public void set(AgroRecipe recipe) {
		lblEvery.setText(EVERYTIME+(recipe.time()/50.0)+SECONDS);
		ItemEntry item = recipe.crop();
		lblIn.setIcon(item.icon());
		lblIn.setText(item.title());
		outList.setListData(VectorUtils.list2arr(recipe.drops()));
	}
